...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.studentClass.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.student-classes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('turno') ? 'is-invalid' : '' }}" name="turno" id="turno" required>
                    ...
                    @foreach(App\Models\StudentClass::TURNO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('turno', 'Selecione o Turno') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('turno'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantidade_de_vagas') ? 'is-invalid' : '' }}" type="number" name="quantidade_de_vagas" id="quantidade_de_vagas" value="{{ old('quantidade_de_vagas', '0') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ano_letivo') ? 'is-invalid' : '' }}" type="number" name="ano_letivo" id="ano_letivo" value="{{ old('ano_letivo', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('serie') ? 'is-invalid' : '' }}" name="serie" id="serie" required>
                    ...
                    @foreach(App\Models\StudentClass::SERIE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('serie', 'Selecione a serie') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('serie'))
